    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (
          ...
          context?.alertBox?.(
            "error",
            "Only JPG, PNG, WEBP or SVG allowed"
          );
          setUploading(false);
          return;
        }
      ...
      if (res?.success) {
        const uploaded = res?.images || res?.data || [];
        onChange?.(uploaded);
      ...
      context?.alertBox?.("error", "Upload error");
    }
  };

  return (
    <div
      className={`relative w-[150px] h-[120px] rounded-md bg-gray-100 p-5 border flex flex-col items-center justify-center ${className}`}
    >
